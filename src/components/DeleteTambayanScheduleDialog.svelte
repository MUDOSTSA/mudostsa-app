<script lang="ts">
  ...
  let {
    ...
    shown: boolean;
    selectedItems: { [key: string]: RoomTambayanSchedule };
    onSuccess?: () => void;
  } = $props();
  ...
  async function handleDelete() {
    ...
    try {
      ...
      for (const id of itemIds) {
        promises.push(deleteRow("tambayan", parseInt(id)));
      }
      ...
      if (failedOperations.length > 0) {
        error = `Failed to delete ${failedOperations.length} schedule entry(s)`;
        return;
      }
      ...
      error = "Failed to delete schedule entries";
...
<Dialog bind:shown title="Delete Schedule Entries" onClose={handleClose}>
  <div class="space-y-4 min-w-96 px-4 py-4">
    ...
    >
      ...
      <p class="text-sm">
        Are you sure you want to delete the selected schedule entr{itemCount > 1
          ? "ies"
        ...
        >
          <div class="flex-1">
            <div class="text-white font-medium">
              Room {item.room} at {item.campus} Campus
            </div>
            <div class="text-sm text-white/60">
              ID: #{item.id}
            </div>
            <div class="text-xs text-white/50 mt-1">
              {new Date(item.time_start).toLocaleString()} - {new Date(
                item.time_end
              ).toLocaleString()}
            </div>
          </div>
    ...
    <div class="text-center text-white/80 text-sm">
      {itemCount} schedule entr{itemCount > 1 ? "ies" : "y"} selected for deletion
    </div>
